...
class HotmartWorker(QThread):
    ...
    def run(self):
        try:
            self.status_changed.emit("START", "Autenticando no Club Hotmart...")
            # TODO: Implementar raspagem das aulas do Hotmart Club / Sparkle
            ...
            traceback.print_exc()
            self.download_error.emit(f"Erro Hotmart: {str(e)}")
