...
class VimeoWorker(QThread):
    ...
    def run(self):
        try:
            self.status_changed.emit("START", "Iniciando captura Vimeo...")
            # TODO: Implementar captura com suporte a vídeos protegidos por Referer / Referrer
            ...
            traceback.print_exc()
            self.download_error.emit(f"Erro Vimeo: {str(e)}")